...
class KMeans:
    ...
    def initialize(self, k):
        ...
        rand_indices = np.random.permutation(self.num_samps)[:k]
        return self.data[rand_indices]

    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False, animate=False):
        ...
        animate: boolean. If set to True, the clustering process will be visualized as an animation
        ...
        if animate:
            scat, cent, fig, ax = self.plot_clusters()
            step = 0
            ...
            def update_plot(frame):
                ...
                scat.set_offsets(self.data)
                scat.set_array(self.data_centroid_labels)
                cent.set_offsets(self.centroids)
            ...
            anim = animation.FuncAnimation(fig, update_plot, frames=max_iter, interval=200)
        ...
        return (self.inertia, step, anim) if (animate) else (self.inertia, step)
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False):
        ...
        verbose: boolean. Print out debug information if set to True.
        '''
        temp_centroids, temp_data_centroid_labels = None, None
        min_inertia = float('inf')
        for i in range(n_iter):
            self.cluster(k=k, verbose=verbose)
            if self.inertia < min_inertia:
                min_inertia = self.inertia
                temp_centroids = self.centroids
                temp_data_centroid_labels = self.data_centroid_labels
        self.centroids = temp_centroids
        self.data_centroid_labels = temp_data_centroid_labels
    ...
    def update_labels(self, centroids):
        ...
        '''
        return np.array( [np.argmin(self.dist_pt_to_centroids(self.data[i, :], centroids)) for i in range(self.data.shape[0])] )
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        labels_count = np.zeros((k, 1))
        for i in range(self.num_samps):
            new_centroids[data_centroid_labels[i], :] += self.data[i, :]
            labels_count[data_centroid_labels[i], :] += 1
        
        # Handle the case where there are no samples assigned to a cluster
        for i in range(k):
            if labels_count[i] == 0:
                random_idx = np.random.randint(self.num_samps)
                new_centroids[i, :] = self.data[random_idx, :]
                labels_count[i] = 1
        
        new_centroids = new_centroids / labels_count
    ...
    def compute_inertia(self):
        ...
        '''
        return np.sum( ( (self.data - self.centroids[self.data_centroid_labels])**2).sum(axis=1) ) / self.num_samps
    ...
    def plot_clusters(self):
        ...
        -----------
        scat. PathCollection
            PathCollection objects of all datapoints
        cent. PathCollection
            PathCollection objects of centroids on plot
        fig. Figure
        ...
        cmap = colorbrewer.diverging.Spectral_11
        fig, ax = plt.subplots()
        scat = ax.scatter(self.data[:, 0], self.data[:, 1], c=self.data_centroid_labels, cmap=cmap.mpl_colormap, edgecolors='black', alpha=.5)
        cent = ax.scatter(self.centroids[:, 0], self.centroids[:, 1], marker='s', linewidths=3, color='white', edgecolors='black')
        plt.colorbar(scat, label='cluster', ticks=[i for i in range(self.centroids.shape[0])])
        plt.show()
        return scat, cent, fig, ax
    ...
    def elbow_plot(self, max_k, n_iter=1):
        ...
        n_inter: number of iteration to run for each kmeans 
        ...
        for k in range(1, max_k + 1, 1):
            self.cluster(k) if n_iter==1 else self.cluster_batch(k, n_iter)
